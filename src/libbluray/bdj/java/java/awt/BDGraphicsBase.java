...
abstract class BDGraphicsBase extends DVBGraphics implements ConstrainableGraphics {
    ...
    private void drawSpanN(int x, int y, int length, int src[], int srcOffset, boolean flipX) {
        ...
        {
//            logger.info("drawSpanN (with flip) correcting length: sourceoffset: " + srcOffset + ", length: "+ length + ", source length: " + src.length);
//            length = srcOffset + length - src.length;
        ...
        if (length <= 0) 
        {
            logger.error("drawSpanN (with flip) length is null");
            return;
        }

        //  test intersect with screen
        Rectangle rect = new Rectangle(x, y, length, 1);
        rect = actualClip.intersection(rect);
        if (rect.width <= 0 || rect.height <= 0 || rect.x < 0 || rect.y < 0 || backBuffer == null) {
//            logger.error("drawSpanN (with flip) removed after clip");
            return;
        }

        int dstOffset;

        //  offset source for clip
        srcOffset += rect.x - x;
//        logger.info("drawSpanN (with flip) corrected sourceoffset to: " + (rect.x - x));
        //  dest x
        x          = rect.x;
        //  length after clip
        length     = rect.width;
...
//        logger.info("drawSpanN (with flip) final: x:" + x + ", y: " + y + ", length: " + length);
    ...
    //  most import for images: draw a line (or point) of pixels on 1 horizontal line in the backbuffer
    ...
    {
//        logger.info("drawImage (observer)");
    ...
    {
//        logger.info("drawImage (color/observer)");
        return drawImageN(img, x, y, -1, -1, 0, 0, -1, -1, false, false, bg, observer);
    }

    /**
     * Draws a scaled image to x,y,w,h in nonblocking mode with a
     * callback object.
     */
    public boolean drawImage(Image img, int x, int y, int w, int h,
        ImageObserver observer) 
    {
//        logger.info("drawImage scaled (observer)");
    ...
    {
//        logger.info("drawImage scaled (color/observer)");
    ...
    public boolean drawImage(Image img,
        ...
        ImageObserver observer) 
    {
//        logger.info("drawImage subrectangle (observer)");
    ...
    public boolean drawImage(Image img,
        ...
        ImageObserver observer) 
    {
//        logger.info("drawImage subrectangle (background/observer)");
    ...
    protected boolean drawImageN(Image img,
        ...
        {
            bdImage = (BDImage)img;
//            logger.info("drawImageN image source is BDImage");
        } 
        else 
...
//            logger.info("drawImageN image source is DVBBufferedImage");
        } 
        else 
        if (img instanceof BufferedImage) 
        {
            bdImage = (BDImage)getBufferedImagePeer((BufferedImage)img);
//            logger.info("drawImageN image source is BufferedImage");
        } 
        else 
        {
        ...
        {
//            logger.info("drawImageN image source is BDImageConsumer");

            BDImageConsumer consumer = (BDImageConsumer)bdImage;
            if (!consumer.isComplete(observer)) 
            {
                logger.error("drawImageN: asking for drawing with consumer, and consumer is not finished.");
                return false;
            }
        }
...
//        logger.info("drawImageN (dest)     : dx:" + dx + ", dy:" + dy + ", dw:" + dw + ", dh:" + dh );
//        logger.info("drawImageN (dest im)  : w: " + width + ", height: " + height);
//        logger.info("drawImageN (source)   : sx:" + sx + ", sy:" + sy + ", sw:" + dw + ", sh:" + sh );
//        logger.info("drawImageN (source im): w: " + bdImage.width + ", height: " + bdImage.height);
//        logger.info("drawImageN flipX: " + flipX + ", flipY: " + flipY);

        //  get the stride
        int   stride   = bdImage.width;
        //  destbuffer we are drawing from
        int[] rgbArray = bdImage.getBdBackBuffer();

        if (bg != null) {
            int  bgColor = bg.getRGB();
        ...
        // resize if needed
        if (dw != sw || dh != sh) {
            drawResizeBilinear(rgbArray, (sy * stride) + sx, stride, sw, sh, dx, dy, dw, dh, flipX, flipY);
            return true;
        }
        ...
        /*
        if((width == 1920) && (height == 1080) && (bdImage.width == 1920) && (bdImage.height == 1080))
        {
            saveRawImage("c:\\temp\\br4\\");
            bdImage.saveRawImage("c:\\temp\\br4\\");
        }
        */
        return true;
    }
    ...
    { 
        ...
        String filename = dst + "dest_c" + counter + "_w" + width + "_h" + height + ".raw";
        counter++;
    ...
    private void drawResizeBilinear(int[] pixels, int offset, int scansize, 
                                    int sw, int sh,
                                    int dx, int dy, int dw, int dh, 
                                    boolean flipX, boolean flipY) {
    ...
    public void dispose() {
//        logger.info("dispose called for BDGraphicsBase");
        tmpLine = null;
        font = null;
        fontMetrics = null;
        gc = null;
