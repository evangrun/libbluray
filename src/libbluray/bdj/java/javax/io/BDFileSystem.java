...
public abstract class BDFileSystem extends FileSystem {
    ...
    static {
        try {
             ...
             * In some Java 9 versions this methods is called in VM init phase
    ...
    */
    ...
    public String normalize(String pathname) {
        return ((BDFileSystem) fs).normalize(pathname);
    }

    public int prefixLength(String pathname) {
        return ((BDFileSystem) fs).prefixLength(pathname);
    }
    ...
    public String resolve(String parent, String child) {
        if (!booted)
            return ((BDFileSystem) fs).resolve(parent, child);
        ...
        String resolvedPath = ((BDFileSystem) fs).resolve(parent, child);
    ...
    public String getDefaultParent() {
        return ((BDFileSystem) fs).getDefaultParent();
    }

    public String fromURIPath(String path) {
        return ((BDFileSystem) fs).fromURIPath(path);
    }

    public boolean isAbsolute(File f) {
        return ((BDFileSystem) fs).isAbsolute(f);
    }
    ...
    public String resolve(File f) {
        if (!booted)
            return ((BDFileSystem) fs).resolve(f);
        ...
        String resolvedPath = ((BDFileSystem) fs).resolve(f);
        String cachePath = BDJLoader.getCachedFile(resolvedPath);
        if (!cachePath.equals(resolvedPath)) {
    ...
    public String canonicalize(String path) throws IOException {
        if (!booted)
            return ((BDFileSystem) fs).canonicalize(path);

        String canonPath = ((BDFileSystem) fs).canonicalize(path);
    ...
    public int getBooleanAttributes(File f) {
        if (!booted)
            return ((BDFileSystem) fs).getBooleanAttributes(f);
    ...
    */
    ...
    public long getLength(File f) {
        if (!booted)
            return ((BDFileSystem) fs).getLength(f);
        ...
        String path = home + f.getPath();
        logger.info("Relative path " + f.getPath() + " translated to " + path);
        return ((BDFileSystem) fs).getLength(new File(path));
    }
    ...
    */
    ...
    public boolean delete(File f) {
        return ((BDFileSystem) fs).delete(f);
    }
    ...
    public boolean createDirectory(File f) {
        return ((BDFileSystem) fs).createDirectory(f);
    }

    public boolean rename(File f1, File f2) {
        return ((BDFileSystem) fs).rename(f1, f2);
    }

    public boolean setLastModifiedTime(File f, long time) {
        return ((BDFileSystem) fs).setLastModifiedTime(f, time);
    }

    public boolean setReadOnly(File f) {
        return ((BDFileSystem) fs).setReadOnly(f);
    }

    public File[] listRoots() {
        return ((BDFileSystem) fs).listRoots();
    }

    public int compare(File f1, File f2) {
        return 0; //fs.compare(f1, f2);
    }
